'''Given an 'N'*'M' integer matrix, if an element is 0, set its entire row and column to 0's and return the matrix.
In particular, your task is to modify it in such a way that if a cell has a value 0(matrix[i][j]==0),
then all the cells od the ith row and jth column should be changed to 0. you must do it in place.
